...
## compile options
CFLAGS += -D_LINUX_  -DARCH_X86_LINUX
...
ifeq ($(LINUX_COMPILER),_DAVINCI_ARMLINUX_)
ifneq ($(DEBUG),1)
  LDFLAGS += --static
endif
endif
...
ifneq ($(strip $(ARC_TARGET)),)
  ...
  install_arc: $(ARC_TARGET)
	$(INSTALL) $(OBJ_DIR)/$(ARC_TARGET) $(INSTALL_LIB_PATH)/$(ARC_TARGET)

  $(ARC_TARGET) : $(OBJS) $(LINKER_FILE)
	$(AR) crus $(OBJ_DIR)/$(ARC_TARGET) $^
...
ifneq ($(strip $(SO_TARGET)),)
  ...
  install_so: $(SO_TARGET)
	$(INSTALL) $(OBJ_DIR)/$(SO_TARGET) $(INSTALL_LIB_PATH)/$(SO_TARGET)
  
  $(SO_TARGET) : $(OBJS) $(LINKER_FILE)
ifeq ($(USE_CXX_AS_LD), 1)
	$(CPP) $^ -o $(OBJ_DIR)/$(SO_TARGET) $(LDFLAGS)
else
	$(LD) $^ -o $(OBJ_DIR)/$(SO_TARGET) $(LDFLAGS)
endif
...
ifneq ($(strip $(APP_TARGET)),)
  ...
  install_app: $(APP_TARGET)
	$(CP) $(APP_TARGET) $(INSTALL_APP_PATH)
  
  $(APP_TARGET): $(OBJS) $(LINKER_FILE)
ifeq ($(USE_CXX_AS_LD), 1)
	$(CPP) $^ -o $(APP_TARGET) $(LDFLAGS)
else
	$(LD) $^ -o $(APP_TARGET) $(LDFLAGS)
endif
